...
export class ChatEntitlementRequests extends Disposable {
	...
	private async resolve(): Promise<void> {
		...
		// Dark Matter: If there is no auth provider, do not attempt to resolve online entitlements
		// which would otherwise downgrade our locally-granted "Free" status back to "Unknown".
...
export class ChatEntitlementContext extends Disposable {
	...
	constructor(
		...
		// Dark Matter: if no auth provider is configured, mark setup as completed so the
		// sign-in screen never appears and the Chat View opens directly.
		const noAuthProvider = !product.defaultChatAgent?.providerExtensionId;
		this._state = this.storageService.getObject<IChatEntitlementContextState>(ChatEntitlementContext.CHAT_ENTITLEMENT_CONTEXT_STORAGE_KEY, StorageScope.PROFILE) ?? {
			entitlement: noAuthProvider ? ChatEntitlement.Free : ChatEntitlement.Unknown,
			organisations: undefined,
			sku: undefined,
			copilotTrackingId: undefined
		};
		if (noAuthProvider) {
			// Mark as installed + completed, bypass the GitHub sign-in flow entirely

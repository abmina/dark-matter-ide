...
function shouldShowBuiltInMode(mode: IChatMode, assignments: { showOldAskMode: boolean }, agentModeDisabledViaPolicy: boolean): boolean {
	...
	if (mode.id === ChatMode.Edit.id || mode.name.get().toLowerCase() === 'edit') {
		if (mode.id === ChatMode.Edit.id) {
			// Show old built-in Edit when policy disables agent mode,
			// OR when there is no custom "edit" mode to replace it (non-Copilot setup)
			return agentModeDisabledViaPolicy || mode.isBuiltin;
		} else {
	...
	if (mode.id === ChatMode.Ask.id || mode.name.get().toLowerCase() === 'ask') {
		if (mode.id === ChatMode.Ask.id) {
			// Show old built-in Ask when assignment says so, policy disables agent,
			// OR when there is no custom "ask" mode to replace it (non-Copilot setup)
			return assignments.showOldAskMode || agentModeDisabledViaPolicy || mode.isBuiltin;
		} else {

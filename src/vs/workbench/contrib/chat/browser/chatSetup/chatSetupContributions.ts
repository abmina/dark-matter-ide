...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		if (!product.defaultChatAgent?.providerExtensionId) {
			...
			if (context) {
				// Mark as fully set up immediately
		...
		const updateRegistration = () => {
			...
			// Agent + Tools
			{
				if (!context.state.hidden && !context.state.disabledInWorkspace) {
					...
					if (product.defaultChatAgent?.providerExtensionId && (!context.state.completed || context.state.entitlement === ChatEntitlement.Unknown || context.state.entitlement === ChatEntitlement.Unresolved) && !vscodeAgentDisposables.value) {
	...
	private async checkExtensionInstallation(context: ChatEntitlementContext): Promise<void> {
		...
		// Dark Matter: when there is no auth provider (i.e., we are running with a local LLM),
		// skip the marketplace extension check entirely. The built-in Dark Matter
		// is always "installed" — there is nothing to poll for.
		if (!product.defaultChatAgent?.providerExtensionId) {
